...
void Init_Video()
{
	...
	sdl_screen = SDL_SetVideoMode(HOST_WIDTH_RESOLUTION, HOST_HEIGHT_RESOLUTION, 16, SDL_SWSURFACE | SDL_FULLSCREEN | SDL_NOFRAME);
...
void Set_Video_InGame()
{
	switch(option.fullscreen) 
	{
        ...
        case 0:
			...
			if (sdl_screen->w != INTERNAL_WSWAN_WIDTH) sdl_screen = SDL_SetVideoMode(224, 144, 16, SDL_SWSURFACE | SDL_FULLSCREEN | SDL_NOFRAME);
			Draw_to_Virtual_Screen = sdl_screen->pixels;
			width_of_surface = sdl_screen->w;
        break;
        #endif
        default:
			if (sdl_screen->w != HOST_WIDTH_RESOLUTION) sdl_screen = SDL_SetVideoMode(HOST_WIDTH_RESOLUTION, HOST_HEIGHT_RESOLUTION, 16, SDL_SWSURFACE | SDL_FULLSCREEN | SDL_NOFRAME);
